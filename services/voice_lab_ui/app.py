...
with gr.Blocks(title="Tali Voice Lab") as demo:
    ...
    with gr.Tabs():
        with gr.Tab("1. Connect"):
            ...
            gr.Markdown("### 5. Active profile")
            gr.Markdown("This dropdown is filled from the backend. Pick the profile you want to use in the next tabs before uploading a sample or generating a new line.")
            profile_selector = gr.Dropdown(label="Active profile", choices=[], value=None)
        ...
        with gr.Tab("2. Profile Lab"):
            ...
            gr.Markdown("Uploads the sample to the active profile selected in the Connect tab.")
            add_sample_button = gr.Button("Upload sample to active profile", variant="primary")
